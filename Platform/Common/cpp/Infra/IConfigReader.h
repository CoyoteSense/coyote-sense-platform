...
// Factory for creating configuration readers
class IConfigReaderFactory {
public:
    virtual ~IConfigReaderFactory() = default;
    virtual std::unique_ptr<IConfigReader> createReader(const std::string& filePath, ConfigFormat format = ConfigFormat::AUTO) = 0;
    virtual std::unique_ptr<ICoyoteConfig> createCoyoteConfig(const std::string& filePath = "") = 0;
    virtual ConfigFormat detectFormat(const std::string& filePath) = 0;
};
