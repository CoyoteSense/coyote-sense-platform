...
function Test-OAuth2Server {
    ...
    try {
        ...
        if (Test-Path $serverScript) {
            ...
            Write-Info "Please start the OAuth2 server manually:"
...
function Invoke-IntegrationTests {
    ...
    $testFilter = "Category=Integration&Category=RealServer"
    
    Write-Info "Test filter: $testFilter"
    
    dotnet test --filter $testFilter --logger "console;verbosity=detailed" --no-build
    
    if ($LASTEXITCODE -eq 0) {
        Write-Success "C# integration tests passed!"
        return $true
    } else {
