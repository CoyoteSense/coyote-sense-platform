...
public class MockOAuth2Server : IDisposable
{
    ...
    private void SetupEndpoints()
    {
        ...
        _server
            ...
            .RespondWith(Response.Create()
                .WithCallback(HandleIntrospectionRequest));

        // Token Revocation Endpoint
        _server
            .Given(Request.Create().WithPath("/oauth2/revoke").UsingPost())
            .RespondWith(Response.Create()
                .WithCallback(HandleRevocationRequest));
    ...
    private ResponseMessage HandleTokenRequest(IRequestMessage request)
    {
    ...
    private ResponseMessage HandleClientCredentialsGrant(string clientId, Dictionary<string, string> formData)
    {
    ...
    private ResponseMessage HandleJwtBearerGrant(string clientId, Dictionary<string, string> formData)
    {
    ...
    private ResponseMessage HandleAuthorizationCodeGrant(string clientId, Dictionary<string, string> formData)
    {
    ...
    private ResponseMessage HandleRefreshTokenGrant(string clientId, Dictionary<string, string> formData)
    {
    ...
    private ResponseMessage HandleIntrospectionRequest(IRequestMessage request)
    {
        var body = request.Body ?? string.Empty;
        var formData = ParseFormData(body);
        ...
        if (tokenInfo == null || !tokenInfo.IsActive)
        {
            ...
            return new ResponseMessage
            {
                ...
                Body = JsonSerializer.Serialize(inactiveResponse)
            };
        }
        ...
        return new ResponseMessage
        {
            ...
            Body = JsonSerializer.Serialize(activeResponse)
        };
    }

    private ResponseMessage HandleRevocationRequest(IRequestMessage request)
    {
        var body = request.Body ?? string.Empty;
        var formData = ParseFormData(body);
    ...
    private string ExtractClientId(IRequestMessage request, Dictionary<string, string> formData)
    {
        // Try form data first
    ...
    private string ExtractClientSecret(IRequestMessage request, Dictionary<string, string> formData)
    {
        // Try form data first
    ...
    private ResponseMessage CreateErrorResponse(HttpStatusCode statusCode, string error, string description)
    {
