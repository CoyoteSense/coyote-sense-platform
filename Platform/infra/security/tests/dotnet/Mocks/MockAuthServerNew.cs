...
public class MockOAuth2Server : IDisposable
{
    ...
    public int Port => _server.Port!.Value;
    ...
    private void SetupEndpoints()
    {
        ...
        _server
            ...
            .RespondWith(Response.Create()
                .WithCallback(req => HandleTokenRequest(req)));

        // Token Introspection Endpoint
        _server
            .Given(Request.Create().WithPath("/oauth2/introspect").UsingPost())
            .RespondWith(Response.Create()
                .WithCallback(req => HandleIntrospectionRequest(req)));

        // Token Revocation Endpoint
        _server
            .Given(Request.Create().WithPath("/oauth2/revoke").UsingPost())
            .RespondWith(Response.Create()
                .WithCallback(req => HandleRevocationRequest(req)));
    ...
    private object HandleTokenRequest(WireMock.RequestMessage request)
    {
        try
        {
    ...
    private object HandleClientCredentialsGrant(string clientId, Dictionary<string, string> formData)
    {
    ...
    private object HandleJwtBearerGrant(string clientId, Dictionary<string, string> formData)
    {
    ...
    private object HandleAuthorizationCodeGrant(string clientId, Dictionary<string, string> formData)
    {
        ...
        _activeTokens[accessToken] = tokenInfo;
    ...
    private object HandleRefreshTokenGrant(string clientId, Dictionary<string, string> formData)
    {
        ...
        _activeTokens[newAccessToken] = newTokenInfo;
    ...
    private object HandleIntrospectionRequest(WireMock.RequestMessage request)
    {
        try
        {
            var body = request.Body ?? string.Empty;
            ...
            if (tokenInfo == null || !tokenInfo.IsActive)
            {
                ...
                return Response.Create()
                    .WithStatusCode(HttpStatusCode.OK)
                    .WithHeader("Content-Type", "application/json")
                    .WithBody(JsonSerializer.Serialize(inactiveResponse));
            }
            ...
            return Response.Create()
                .WithStatusCode(HttpStatusCode.OK)
                .WithHeader("Content-Type", "application/json")
                .WithBody(JsonSerializer.Serialize(activeResponse));
        }
        catch (Exception ex)
        {
            return CreateErrorResponse(HttpStatusCode.InternalServerError, "server_error", ex.Message);
    ...
    private object HandleRevocationRequest(WireMock.RequestMessage request)
    {
        try
        {
            var body = request.Body ?? string.Empty;
    ...
    private string ExtractClientId(WireMock.RequestMessage request, Dictionary<string, string> formData)
    {
        // Try form data first
    ...
    private string ExtractClientSecret(WireMock.RequestMessage request, Dictionary<string, string> formData)
    {
        // Try form data first
    ...
    private object CreateErrorResponse(HttpStatusCode statusCode, string error, string description)
    {
