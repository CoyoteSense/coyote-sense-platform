...
using GrantType = auth::GrantType;
using AuthMode = auth::AuthMode;
using TokenRequest = auth::TokenRequest;
using TokenResponse = auth::TokenResponse;
using ErrorResponse = auth::ErrorResponse;
using IntrospectResponse = auth::IntrospectResponse;
using ClientConfig = auth::ClientConfig;
using AuthCodeRequest = auth::AuthCodeRequest;
using AuthCodeResponse = auth::AuthCodeResponse;
using JwtBearerClaims = auth::JwtBearerClaims;

// Legacy OAuth2 type aliases
using OAuth2GrantType = auth::GrantType;
using OAuth2AuthMode = auth::AuthMode;
using OAuth2TokenRequest = auth::TokenRequest;
using OAuth2TokenResponse = auth::TokenResponse;
using OAuth2ErrorResponse = auth::ErrorResponse;
using OAuth2IntrospectResponse = auth::IntrospectResponse;
using OAuth2ClientConfig = auth::ClientConfig;
using OAuth2AuthCodeRequest = auth::AuthCodeRequest;
using OAuth2AuthCodeResponse = auth::AuthCodeResponse;
using OAuth2JwtBearerClaims = auth::JwtBearerClaims;
