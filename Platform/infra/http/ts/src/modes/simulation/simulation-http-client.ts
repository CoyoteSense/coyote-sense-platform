...
import { HttpClientOptions, SimulationModeOptions, DEFAULT_HTTP_OPTIONS } from '../../interfaces/configuration';
import { BaseHttpClient, HttpResponseImpl } from '../../interfaces/base-http-client';

/** Quick config overload */
...
// Alias for external usage in tests
...
export class SimulationHttpClient extends BaseHttpClient {
  ...
  // Overloads
  constructor(opts: HttpClientOptions, simOpts: SimulationModeOptions, logger?: Console);
  constructor(cfg: SimulationHttpConfig, logger?: Console);
  constructor(
    a: HttpClientOptions|SimulationHttpConfig,
    b?: SimulationModeOptions|Console,
    c?: Console
  ) {
  ...
  public getStats() {
    const def = this.defaultScenarios.length;
    const cust = this.scenarios.length;
    return { defaultScenarios: def, customScenarios: cust, totalScenarios: def + cust };
  }
   ...
   */
  public async sendAsync(request: HttpRequest): Promise<HttpResponse> {
  ...
  protected override async onDispose(): Promise<void> {
    // Simulation HTTP client doesn't need special cleanup
