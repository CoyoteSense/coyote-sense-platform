...
import { HttpClientFactory, HttpClientContainer, createClient, createHttpClientContainer } from '../src/factory/http-client-factory';
...
describe('Convenience functions', () => {
  ...
  describe('createClient', () => {
    it('should create client with default mode (REAL)', () => {
      const client = createClient(undefined, mockLogger);
      expect(client).toBeInstanceOf(RealHttpClient);
    });

    it('should create client with specified mode', () => {
      const client = createClient(RuntimeMode.MOCK, mockLogger);
      expect(client).toBeInstanceOf(MockHttpClient);
    });
  });
});
